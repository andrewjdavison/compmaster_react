...
describe('TableClass', function(){
  describe('finding elements in a table', ()=>{
    var result=null;
    var token=null;
    before((done)=>{
    ...
    it('returns a single result when the model contains a singlular exemplar', (done)=>{

      var club = new Club();

      club.find({id:1})
      .then((res)=> {
        assert.equal(club.name,'Melbourne Brewers');
      });
    });
    it.skip('Authenticates', (done)=>{
      // Authenticate first
      request(app)
        .post('/auth')
    ...
    it.skip('responds with a 200 OK and an array with 3 elements', (done)=>{
      request(app)
        .get('/clubs')
