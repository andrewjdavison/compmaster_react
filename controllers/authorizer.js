...
auth.role('owner', function(req, done){
  // If this was a request for user data, the param ID will have been userId.
...
auth.role('compmanager', function(req, done){
  console.log('Testing to see if this is a comp admin for a specific competition');
  var compInstId ;
  if(req.params.hasOwnProperty('compInstId')){
    compInstId = req.params.compInstId;
  }
  if(req.query.hasOwnProperty('compInstId')){
    compInstId = req.query.compInstId;
  }
  if(!compInstId) {
    if(req.body.hasOwnProperty('entry')){
      compInstId = req.body.entry.compInstId;
    }
    // Now it's possible that the ID has been inverted to "cancel" it, so check for that!
  ...
  var query = "SELECT * FROM compauth WHERE `compInstId`='"+compInstId+
               "' AND `userId`='"+userId+"';";
  console.log(query);
  console.log(compInstId);
  console.log(cmconst.PermClass.judgeAdmin);
  console.log(cmconst.PermClass.compAdmin);
  db.query(query)
  .then((result)=>{
    if(result[0].length ==0 ){
...
auth.role('anycompmanager', function(req, done){
  console.log('Testing to see if this is a comp admin');

  var userId = req.user.userid;
  var authorized=false;

  var query = "SELECT * FROM compauth WHERE `userId`='"+userId+"';";
  db.query(query)
  .then((result)=>{
    if(result[0].length ==0 ){
      console.log('No result!');
...
auth.action('read own data', ['owner','admin']);
...
auth.action('read user lists', ['admin', 'anycompmanager']);
